...
export class PolymarketExecutor {
  ...
  constructor(market, source, { mode = 'paper', sizeUsd = 5 } = {}) {
    this.m = market;
    this.src = source;
    this.mode = mode;
  ...
  async _liveClient() {
    ...
    const boot = new ClobClient(CLOB, 137, signer);
    const creds = await boot.createOrDeriveApiKey();
    this._client = new ClobClient(CLOB, 137, signer, creds);
    return this._client;
  }
  ...
  state() {
    return {
      ...
      realizedUsd: +this.orders.filter(o => o.pnlUsd != null).reduce((a, o) => a + o.pnlUsd, 0).toFixed(2)
    };
  }
}
