...
const SERIES_WC = '11433'; // soccer-fifwc
const MATCH_SLUG = /^fifwc-[a-z]{2,4}-[a-z]{2,4}-\d{4}-\d{2}-\d{2}$/;

/** the real World Cup schedule, straight from the venue */
export async function upcomingMatches() {
  const evs = await fetch(`${GAMMA}/events?series_id=${SERIES_WC}&closed=false&limit=100`).then(r => r.json());
  const now = Date.now();
  return evs
    .filter(e => MATCH_SLUG.test(e.slug || ''))
    .map(e => {
      const [home, away] = (e.title || '').split(/ vs\.? /i);
      return {
        fixtureId: e.slug, polymarketSlug: e.slug,
        home, away, kickoff: e.endDate,
        live: new Date(e.endDate).getTime() <= now,
...
/** No World Cup left to trade (the judges' review window): fall back to the
 *  most active binary market on the venue so Live mode always works. */
export async function anyActiveMarket() {
  const ms = await fetch(`${GAMMA}/markets?closed=false&active=true&order=volume24hr&ascending=false&limit=25`)
    .then(r => r.json());
  const m = ms.find(x => x.clobTokenIds && !x.negRisk) || ms.find(x => x.clobTokenIds);
  if (!m) throw new Error('no active Polymarket market found');
  const [yes, no] = JSON.parse(m.clobTokenIds);
  const [home, away] = (m.question || '').split(/ vs\.? /i);
  return {
    eventTitle: m.question, question: m.question, marketId: m.id,
    yes, no, fixtureId: `poly-${m.id}`, home, away,
    tickSize: String(m.orderPriceMinTickSize || '0.01'),
    negRisk: !!m.negRisk,
    label: m.question, fallback: true
  };
}
...
export async function findMarket(slug, questionLike) {
  ...
  return {
    ...
    yes, no, fixtureId: `poly-${m.id}`, home, away,
    // live orders are rejected with the wrong tick size or negRisk flag —
    // take both from the market itself, never hardcode
    tickSize: String(m.orderPriceMinTickSize || '0.01'),
...
export class PolymarketExecutor {
  ...
  async _order(action, tokenName, side, price, shares, signalId) {
    ...
    try {
      ...
      const resp = await client.createAndPostOrder(
        ...
        { tickSize: this.m.tickSize || '0.01', negRisk: !!this.m.negRisk },
        OrderType.FAK
      );
  ...
  // shorting the outcome = buy NO, long = buy YES (works for fade AND ride:
  // the signal's tdir already encodes which way the trade points)
  async onEntered(sig) {
    ...
    const wantNo = (sig.tdir ?? -sig.direction) === -1;
  ...
  state() {
    return {
      ...
      // funder is public info (the deposit address); abbreviated for display
      funder: (() => {
        const f = this._funder || process.env.POLY_FUNDER;
        return f ? `${f.slice(0, 6)}…${f.slice(-4)}` : null;
      })(),
...
export class MarketMaker {
  ...
  start() {
    ...
    // quote immediately off the last known mid instead of waiting for the next
    // 3s book poll (users read —/— as broken)
    const mid = this.src?.lastBook?.yes?.mid;
    if (mid != null) this._tick({ price: mid });
  }
  async stop() {
    this.running = false;
    ...
    try {
      ...
      for (const [price, side] of [[bid, Side.BUY], [ask, Side.SELL]]) {
        ...
        const resp = await client.createAndPostOrder(
          ...
          { tickSize: this.m.tickSize || '0.01', negRisk: !!this.m.negRisk }, OrderType.GTC
        );
