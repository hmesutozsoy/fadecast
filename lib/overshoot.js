...
export const DEFAULTS = {
  ...
  side: 'fade'     // 'fade' trades against the panic (mean reversion);
                   // 'ride' trades with it (momentum) — same detector, opposite bet
};
...
export class OvershootEngine extends EventEmitter {
  ...
  addTick({ fixtureId, outcome, price, ts, meta }) {
    ...
    // bound memory for indefinite live operation — the strategy only ever
    // looks back lookback+confirm+hold seconds, far less than 1200 ticks
    if (s.ticks.length > 2400) s.ticks.splice(0, 1200);
    if (this.signals.length > 400) this.signals.splice(0, 100);
  ...
  _detect(s) {
    ...
    const signal = {
      ...
      strategy: this.p.side,
      // tdir: which way the TRADE points (+1 long the outcome, -1 short).
      // Fading opposes the jump; riding follows it.
      tdir: this.p.side === 'ride' ? direction : -direction,
      side: this.p.side === 'ride'
        ? (direction === 1 ? 'RIDE_UP (buy)' : 'RIDE_DOWN (sell)')
        : (direction === 1 ? 'FADE_UP (sell)' : 'FADE_DOWN (buy)'),
      pre: round4(ref),
      det: round4(last.p),
      jump: round4(jump),
      panic,
  ...
  _resolveOpenSignals(now) {
    ...
    for (const sig of this.signals) {
      ...
      if (sig.status === 'open') {
        ...
        // enter once the cascade exhausts itself, and only inside the band
        if (now - sig.extremeT >= confirm && cur >= band[0] && cur <= band[1]) {
          // tdir +1 buys at the ask, tdir -1 sells at the bid
          sig.entry = round4(sig.tdir === 1 ? cur + halfSpread : cur - halfSpread);
          sig.entryT = now;
          sig.status = 'entered';
      ...
      if (sig.status === 'entered') {
        const adverse = sig.tdir === 1 ? sig.entry - cur : cur - sig.entry;
        const stopped = adverse >= stopLoss;
        if (stopped || now >= sig.entryT + hold) {
          const mid = stopped ? cur : this.priceAt(s, sig.entryT + hold);
          if (mid !== null) {
            sig.exit = round4(sig.tdir === 1 ? mid - halfSpread : mid + halfSpread);
            sig.pnl = round4(sig.tdir === 1 ? sig.exit - sig.entry : sig.entry - sig.exit);
            sig.stopped = stopped;
            sig.status = 'resolved';
