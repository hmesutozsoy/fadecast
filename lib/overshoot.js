...
export const DEFAULTS = {
  ...
  hold: 240,       // s until the fade exit is priced (signal resolution)
  ...
  _resolveOpenSignals(now) {
    ...
    for (const sig of this.signals) {
      ...
      if (sig.status === 'entered') {
        const { stopLoss } = this.p;
        const cur = this.priceAt(s, now);
        const adverse = cur === null ? 0
          : sig.direction === 1 ? cur - sig.entry : sig.entry - cur;
        const stopped = adverse >= stopLoss;
        if (stopped || now >= sig.detT + entryLag + hold) {
          const mid = stopped ? cur : this.priceAt(s, sig.detT + entryLag + hold);
          if (mid !== null) {
