...
const T = {
  goal: [
    "GOAL {team} at {min}'. And here comes the stampede — watch the market trip over itself.",
    "{team} scores. The market is about to price this like the game just ended. It didn't.",
    "Goal, {team}, {min}'. Somewhere out there, a thousand bettors just slammed the buy button at the top.",
    "{team} finds the net. Deep breaths, everyone. The market won't take its own advice."
  ],
  signal_high: [
...
const MOODS = { goal: '⚽', signal_high: '🚨', signal_mid: '📉', entered: '🎯', win: '💰', loss: '🩹' };
...
export class Pundit {
  ...
  goal({ team, minute, meta, ts }) { return this.say('goal', { team, min: minute, label: meta?.label, ts }); }
  signal(s) {
  ...
  resolved(s) {
    return this.say(s.pnl > 0 ? 'win' : 'loss',
