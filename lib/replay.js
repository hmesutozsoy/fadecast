...
const MATCHES = [
  { fixtureId: 'wc2026-qf-1', home: 'Brazil', away: 'France', p0: 0.52, goals: [{ min: 23, team: 'away' }, { min: 61, team: 'home' }, { min: 78, team: 'home' }] },
  { fixtureId: 'wc2026-qf-2', home: 'Argentina', away: 'England', p0: 0.47, goals: [{ min: 12, team: 'home' }, { min: 84, team: 'away' }] },
  { fixtureId: 'wc2026-qf-3', home: 'Spain', away: 'Morocco', p0: 0.61, goals: [{ min: 55, team: 'away' }] }
];

export function* syntheticTicks(seed = 7) {
  ...
  for (const m of MATCHES) {
    ...
    const meta = { home: m.home, away: m.away, label: `${m.home} vs ${m.away}` };
    let fair = m.p0;
    ...
    for (let sec = 0; sec <= 95 * 60; sec += 4 + Math.floor(rand() * 5)) {
      ...
      for (const g of m.goals) {
        ...
        if (sec >= gs && !fired.has(g)) {
          ...
          over += dir * (0.05 + rand() * 0.05);             // the market panics past fair
          all.push({
            kind: 'goal', fixtureId: m.fixtureId, ts: gs, meta,
            team: g.team === 'home' ? m.home : m.away, minute: g.min
          });
        }
      }
      all.push({
        fixtureId: m.fixtureId, outcome: 'home',
        price: clamp(fair + over + noise), ts: sec, meta
      });
    }
  }
...
export class ReplaySource extends EventEmitter {
  ...
  constructor({ speed = 30, recording = null, seed = 7 } = {}) {
    super();
    this.speed = speed;
    this.gen = recording ? recordedTicks(recording) : syntheticTicks(seed);
    this.stopped = false;
  }
  ...
  start() {
    ...
    const step = () => {
      ...
      clock = value.ts;
      setTimeout(() => {
        this.emit(value.kind === 'goal' ? 'score' : 'tick', value);
        step();
      }, Math.min(wait, 500));
    };
    step();
